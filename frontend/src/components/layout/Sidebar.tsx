...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
    ...
    return (
        <>
            {/* --- MOBILE OVERLAY AND BUTTON --- */}
            <button
                className="md:hidden fixed top-4 left-4 z-50 bg-indigo-700 p-2 rounded-lg text-white shadow-lg"
                        ...
                        {navItems.map((item) => (
                            <li key={item.to}>
                                <NavLink
                                    to={item.to}
                                    className={({ isActive }) =>
                                        `flex items-center py-3 rounded-lg transition-all duration-300 relative ${isActive ? "bg-indigo-600/90 text-white shadow-md" : "text-gray-300 hover:bg-indigo-600/30 hover:text-white"} ${isOpen ? 'px-4' : 'justify-center px-0'}`
                                    }
                                >
                                    {({ isActive }) => (
                                        <>
