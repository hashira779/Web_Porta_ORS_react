...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
    ...
    const isItemActive = (item: NavItem) => {
        return location.pathname === item.to ||
            (item.subItems && item.subItems.some(subItem =>
                location.pathname === subItem.to // Exact match for dynamic routes
            ));
    };
    ...
    // Conditionally render sidebar only if there are navigable items
    const hasNavigableItems = filteredNavItems.length > 0 || (currentUser && currentUser.role?.name === 'admin');
    ...
    return (
        <>
            ...
            {/* Mobile overlay (only show if there are items) */}
            <AnimatePresence>
                {isOpen && hasNavigableItems && (
                    <motion.div
            ...
            {/* Sidebar (only render if there are navigable items) */}
            {hasNavigableItems && (
                <motion.aside
                    initial={false}
                    animate={{
               ...
               ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} border-r border-gray-700/30`}
                ...
                >
                    ...
                    {/* User profile and logout (only show if user is logged in) */}
                    {currentUser && (
